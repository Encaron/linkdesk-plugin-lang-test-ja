# 日本語テスト（lang-test-ja）——LinkDesk 插件仓

> **本文件是给在这个仓里干活的 AI 看的**（Claude Code / Codex / Cursor / …）。人看 `README.md`。
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`lang-test-ja`）。当前版本 `1.0.4`。

## 1. 这是什么

日语测试语言包——**验证载体**，故意只译了一部分。
...
`ja.json` 在**仓根**；本仓**没有 `resources/`**，`plugin.json` 里也**没有 `icon`**。
